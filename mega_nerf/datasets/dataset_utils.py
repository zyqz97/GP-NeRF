...
def get_rgb_index_mask(metadata: ImageMetadata) -> Optional[
    ...
    #     import numpy as np
    #     import cv2
    #     mask_shape = keep_mask[keep_mask == True].shape[0]
    #     if mask_shape != keep_mask.shape[0] * keep_mask.shape[1] and mask_shape !=0:
    #         visual_rgb = metadata.load_image().view(keep_mask.shape[0], keep_mask.shape[1], 3)
    #         cv2.imshow('2', visual_rgb.numpy()[:,:, ::-1])
    #         cv2.waitKey()
    #         cv2.destroyAllWindows()
    #         visual_rgb = (visual_rgb * keep_mask.unsqueeze(-1).repeat(1,1,3))
    #         cv2.imshow('3', visual_rgb.numpy()[:,:, ::-1])
    #         cv2.waitKey()
    #         cv2.destroyAllWindows()
    #         print(mask_shape)
